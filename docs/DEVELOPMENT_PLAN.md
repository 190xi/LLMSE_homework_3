...
- ✅ Week 4: 语音输入功能提前实施（原Week 5）
- ✅ Week 7: 多方案生成提前实施（原Week 14）
- ✅ Week 12: 补充实时导航功能（新增）
- ✅ Phase 4: 补充图片识别和多语言详细规划

> 调整后的时间线更符合PRD功能优先级，确保P0/P1核心功能在MVP阶段完整交付
...
- [x] **第三方服务集成**
  - 阿里云通义千问API测试（已完成）
  - 科大讯飞账号注册及API Key获取（已完成）
  - 高德地图账号注册及API Key获取（部分完成）
  - Supabase连接配置（已完成）
- [ ] **开发工具配置**
  - Husky + lint-staged设置
...
**2.3 语音识别集成 (Week 4)** ⭐ **PRD P0核心功能**

> **调整说明**: 根据PRD 3.1.1节，语音输入是"必需功能"，作为核心交互方式应在用户系统完成后立即集成
...
- [x] **第三方服务配置**
  ...
  - WebSocket连接配置
- [ ] **前端组件开发**
  - 语音录制组件 (`VoiceRecorder.tsx`)
  - 录音波形动画 (`WaveformAnimation.tsx`)
  - 语音输入容器组件 (`VoiceInput.tsx`)
  - 录音状态管理（录音中、识别中、完成）
- [ ] **语音识别功能**
  - 实时语音转文字
  - 识别结果实时显示
...
**3.4 多方案生成与对比 (Week 7)** ⭐ **PRD P1功能提前**

> **调整说明**: 根据PRD 3.1.4节，多方案对比是P1重要功能，应在MVP阶段完成，从Week 14提前到Week 7

- [ ] **AI多方案生成**
  - 修改Prompt支持生成多个方案
  - 生成2-3个不同风格的行程（经济型/均衡型/舒适型）
  - 方案差异化设计（不同景点组合、不同预算分配）
  - 方案生成性能优化（并发生成）
- [ ] **方案对比界面**
  - 方案卡片展示
  - 关键指标对比（总预算、景点数量、行程强度）
  - 方案切换功能
  - 方案详情预览
- [ ] **方案选择与保存**
  - 用户选择偏好方案
  ...
  - 方案切换历史记录
- [ ] **API接口**
  - `POST /api/trips/generate-multiple` - 生成多个方案
  - `PUT /api/trips/:id/select-plan` - 选择方案
...
**6.3 实时导航集成 (Week 12)** ⭐ **PRD P1功能补充**

> **调整说明**: 根据PRD 4.1.2节，实时导航是P1重要功能，需要明确实现方案
...
- [ ] **导航交互**
  ...
  - 导航历史记录
- [ ] **API接口**
  - `GET /api/navigation/route` - 获取路线规划
  - `GET /api/navigation/traffic` - 获取实时路况
...
- [ ] **协作编辑（可选）** ⚠️ **非PRD需求**
  ...
  > **说明**: 此功能不在PRD范围内，属于功能扩展。如需实现，应先更新PRD文档获得确认
  - 邀请同行者（如实现）
  - 实时协作（如实现）
  - 权限管理（如实现）
...
#### P2功能：图片识别费用 ⭐ **PRD 3.2.2节补充**

> **补充说明**: 根据PRD要求，需要支持"拍照记录：扫描小票/发票自动识别金额"

- [ ] **OCR服务选型与集成**
  - 选择OCR服务（推荐：阿里云文字识别/腾讯云OCR）
  - API接口集成
  - 小票/发票识别模型选择
- [ ] **图片上传功能**
  - 拍照上传组件
...
- [ ] **费用自动填充**
  ...
  - 识别准确率统计
- [ ] **API接口**
  - `POST /api/ocr/receipt` - 小票识别
  - `POST /api/expenses/from-image` - 通过图片创建费用

#### P2功能：多语言支持 ⭐ **PRD 5.5节补充**

> **补充说明**: PRD要求支持多语言（中文、英文为优先）

- [ ] **i18n框架集成**
  - 选择i18n方案（推荐：next-intl）
  - 配置语言切换机制
  - 设置默认语言和备用语言
- [ ] **文案翻译**
  - UI界面文案中英文翻译
...
- 根据PRD对照分析进行重大调整
- **功能时间线调整**:
  - 将语音输入功能从Week 5提前到Week 4（PRD P0核心功能）
  - 将多方案生成从Week 14提前到Week 7（PRD P1重要功能）
  - 在Week 12补充实时导航集成（PRD P1功能遗漏）
- **功能补充**:
  - Phase 4新增图片识别费用详细规划（PRD 3.2.2节）
  - Phase 4完善多语言支持实施细节（PRD 5.5节）
- **功能标注**:
  - 标注协作编辑为非PRD需求，避免范围蔓延
- **优化Week 14**: 移除多方案生成，优化AI优化建议功能细节
- 提升PRD与开发计划匹配度至95%以上
...
本次更新基于PRD文档的全面对照分析，发现原开发计划在功能优先级和时间安排上存在以下问题：

1. **语音输入功能安排过晚**: PRD 3.1.1节明确标注语音输入为"必需功能"，是核心交互方式，但原计划安排在Week 5，不符合P0优先级
2. **实时导航功能遗漏**: PRD 4.1.2节的P1功能完全未在开发计划中体现
3. **多方案生成时间过晚**: PRD 3.1.4节的P1功能被安排在Week 14，应该在MVP阶段完成
4. **P2功能规划不够详细**: 图片识别费用和多语言支持缺乏具体实施细节

### 14.2 主要调整内容

#### 调整1: 语音输入功能提前 (Week 5 → Week 4)

**原计划**:

- Week 5: 需求输入界面中包含语音识别集成

**调整后**:

- Week 4: 独立完成语音识别SDK集成和组件开发
- Week 5: 在行程创建向导中复用语音组件

**理由**:

- PRD将语音输入标注为"必需功能"和"核心交互方式"
- 作为P0功能，应该在用户系统完成后立即开发
- 语音组件可以在多个场景复用（行程创建、费用记录）

**影响**:

- Week 4工作量增加，需要额外1-2天开发时间
- Week 5可以专注于表单和行程生成，不受语音集成干扰
- 提前验证语音识别效果，降低后期风险

#### 调整2: 多方案生成提前 (Week 14 → Week 7)

**原计划**:

- Week 14: AI优化建议中包含多方案生成

**调整后**:

- Week 7: AI行程生成后立即实现多方案对比
- Week 14: 专注于路线优化、时间调整、预算优化

**理由**:

- PRD 3.1.4节将多方案对比列为P1功能
- 用户在首次生成行程时就需要对比选择，不应延后
- MVP阶段应该包含完整的行程生成和选择流程

**影响**:

- Week 7工作量增加，需要额外2-3天
- 用户体验显著提升，满足"选择困难症"用户需求
- AI Prompt需要优化，支持生成多个差异化方案

#### 调整3: 补充实时导航功能 (Week 12新增)
...
- Week 12: 地图集成 + 行程地图视图 + 实时导航集成

**理由**:

- PRD 4.1.2节明确要求"一键导航到下一个目的地、集成高德/百度地图导航功能"
- 原开发计划完全遗漏此功能
- 实时导航是P1重要功能，直接影响用户旅行体验

**影响**:

- Week 12工作量增加，需要额外1-2天
- 需要深度集成高德地图导航API
- 移动端需要支持跳转到地图APP

#### 调整4: Phase 4 P2功能详细规划

**图片识别费用（新增详细规划）**:

- OCR服务选型（阿里云文字识别/腾讯云OCR）
- 图片上传、信息提取、自动填充完整流程
- API接口设计（`POST /api/ocr/receipt`）
...
- 原计划: Week 16可选功能
- 调整后: 明确标注为"非PRD需求"，避免范围蔓延
- 如需实施，需先更新PRD文档获得确认

### 14.3 调整后的功能覆盖率

| 优先级   | PRD功能数 | 开发计划覆盖 | 覆盖率      |
| -------- | --------- | ------------ | ----------- |
| P0       | 7         | 7            | 100% ✅     |
| P1       | 6         | 6            | 100% ✅     |
| P2       | 4         | 4            | 100% ✅     |
...
**v1.0版本**: 覆盖率约85%（遗漏实时导航、图片识别费用规划不详细）
**v1.1版本**: 覆盖率达到100%，PRD要求完全落地

### 14.4 时间线对比

```
【v1.0时间线】                     【v1.1调整后时间线】
Week 1-2: 基础架构               Week 1-2: 基础架构 ✅
Week 3-4: 用户系统               Week 3-4: 用户系统 + 语音输入 🔄
Week 5:   需求输入+语音           Week 5:   需求输入（复用语音）
Week 6:   AI生成                 Week 6:   AI生成 ✅
Week 7:   行程展示               Week 7:   行程展示 + 多方案 🔄
Week 8-9: 费用管理               Week 8-9: 费用管理 ✅
Week 10:  MVP测试                Week 10:  MVP测试 ✅
Week 11-12: 地图                 Week 11-12: 地图 + 导航 🔄
Week 13-14: 行程编辑+多方案       Week 13-14: 行程编辑+AI优化 🔄
Week 15-16: 导出分享             Week 15-16: 导出分享 ✅
Week 17+: P2/P3功能（简略）       Week 17+: P2/P3功能（详细）🔄
```
...
**风险**: 语音功能提前可能导致Week 4任务紧张
**应对**:

- 语音SDK集成可以在Week 3就开始准备（申请API Key、阅读文档）
- 语音组件开发可以与用户偏好管理并行
- 如果时间不够，可以将语音组件的高级功能（波形动画）延后

#### 风险2: Week 7工作量增加

**风险**: 多方案生成增加AI调用成本和开发时间
**应对**:

- 使用更便宜的模型（qwen-turbo）生成备选方案
- 采用并发生成提高效率
- MVP阶段可以先实现2个方案，后续扩展到3个
...
1. **立即行动**:
   - [ ] 团队同步本次调整内容
   - [ ] 更新项目看板（Jira/Trello）任务安排
   - [ ] 调整Sprint计划，确保Week 4包含语音功能

2. **Week 3准备**:
   - [ ] 注册科大讯飞账号，获取API Key（如未完成）
   - [ ] 阅读语音SDK文档，准备技术方案
   - [ ] 设计语音组件原型

3. **持续监控**:
   - [ ] 每周回顾进度，确保调整后的计划按时完成
   - [ ] 监控关键里程碑（语音集成、多方案生成、导航功能）
   - [ ] 如有延期风险，及时调整优先级

### 14.7 总结

本次v1.1更新是基于PRD的全面对照分析做出的重要调整，主要成果：

✅ **提升功能覆盖率**: 从85%提升到100%
✅ **优化功能优先级**: P0/P1核心功能在MVP阶段完整交付
✅ **补充遗漏功能**: 实时导航、图片识别、多语言详细规划
✅ **提升开发质量**: 时间安排更合理，降低后期返工风险
✅ **增强文档可执行性**: 每个功能都有清晰的任务清单和交付标准

调整后的开发计划与PRD完全对齐，为项目成功交付奠定坚实基础。
...
1. [x] 完成数据库Schema设计和部署
2. [x] 实现用户注册登录功能
3. [ ] 集成NextAuth.js
4. [x] 创建基础UI组件库
5. [ ] 第一次Sprint回顾

### 第二个Sprint (Week 3-4) ⭐ **含语音功能调整**

1. [ ] 完成用户偏好管理功能
2. [ ] **集成科大讯飞语音识别SDK**（提前实施）
3. [ ] **开发语音输入组件**（提前实施）
4. [ ] 完成认证系统完整流程
5. [ ] Sprint回顾和调整

### 第三个Sprint (Week 5-7) ⭐ **含多方案生成调整**

1. [ ] 完成行程创建向导页面（集成语音输入）
2. [ ] AI行程生成引擎开发和Prompt优化
3. [ ] 行程展示页面开发
4. [ ] **多方案生成与对比功能**（提前实施）
5. [ ] Sprint回顾
